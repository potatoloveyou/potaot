...
router.get('/article', async (ctx, next) => {
	try {
		...
		const articlesJointUsers = await articles
			.aggregate([
				{
					$addFields: {
				...
				},
				{
					$lookup: {
						from: 'users', // 关联的集合
						localField: 'authorIdObjectId', // 使用转换后的 authorIdObjectId 字段
				...
				{
					$sort: { createAt: -1 }, // 按照 createAt 字段降序排序
				},
				{
					$skip: Number(limit) * Number(offset),
				},
			...
			.toArray();
		console.log(articlesJointUsers);

		ctx.body = {
			// pages: Math.ceil((await articles.count()) / Number(limit)),
			pages: Math.ceil((await articles.countDocuments({}, { hint: '_id_' })) / Number(limit)),
			current: Number(offset) + 1,
			list: articlesJointUsers,
		};
	} catch (error) {
		ctx.body = {
			message: '查询失败',
